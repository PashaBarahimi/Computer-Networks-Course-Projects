...
template <>
struct TypeDesc<std::vector<std::string>> {
    static const char* Name() { return "<list of strings>"; }
};
...
public:
     ...
     */
    Cli(std::unique_ptr<Menu> _rootMenu, std::unique_ptr<HistoryStorage> historyStorage = std::make_unique<VolatileHistoryStorage>()) : globalHistoryStorage(std::move(historyStorage)),
...
public:
    ...
    void Enable() {
        if (descriptor) descriptor->Enable();
    }
    void Disable() {
        if (descriptor) descriptor->Disable();
    }
    void Remove() {
...
private:
    struct Descriptor {
        Descriptor() = default;
        Descriptor(std::weak_ptr<Command> c, std::weak_ptr<CmdVec> v) : cmd(std::move(c)), cmds(std::move(v)) {}
        void Enable() {
            if (auto c = cmd.lock())
                c->Enable();
        }
...
public:
    ...
    explicit Menu(const std::string& _name, std::string desc = "(menu)") : Command(_name), parent(nullptr), description(std::move(desc)), cmds(std::make_shared<Cmds>()) {}
...
inline CliSession::CliSession(Cli& _cli, std::ostream& _out, std::size_t historySize) : cli(_cli),
                                                                                        coutPtr(Cli::CoutPtr()),
